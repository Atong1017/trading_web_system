...
def test_boolean_ui_fix():
    """測試布林值參數 UI 修正"""
    print("=== 測試布林值參數 UI 修正 ===")
    
    # 模擬網頁編輯器中新增的布林值參數（包含不同的預設值）
    web_parameters = {
        "enable_stop_loss": {
            "type": "boolean",
            "label": "啟用停損",
        ...
        "use_ma_filter": {
            "type": "boolean",
            "label": "使用均線過濾",
            "default": 1,      # 數字 1（應該被視為 true）
            "description": "是否使用均線過濾"
        },
        "debug_mode": {
            "type": "boolean",
            "label": "除錯模式",
            "default": "true", # 字串 "true"
...
def should_entry(stock_data, current_index, excel_pl_df, **kwargs):
    ...
    print(f"should_entry - 停損:{enable_stop_loss}, 停利:{enable_take_profit}, 均線過濾:{use_ma_filter}, 除錯:{debug_mode}")
    ...
    if current_index > 0:
        ...
        if current_price > prev_price:
            return True, {"reason": f"價格上漲，停損:{enable_stop_loss}, 停利:{enable_take_profit}, 均線過濾:{use_ma_filter}, 除錯:{debug_mode}"}
...
def should_exit(stock_data, current_index, position, excel_pl_df, **kwargs):
    ...
    print(f"should_exit - 停損:{enable_stop_loss}, 停利:{enable_take_profit}, 均線過濾:{use_ma_filter}, 除錯:{debug_mode}")
...
"""
    ...
    print(f"布林值參數配置:")
    for key, config in strategy.custom_parameters.items():
        print(f"  {key}: {config['default']} (類型: {type(config['default'])})")
    ...
    print("\n=== 測試完成 ===")
    print("修正內容:")
    print("1. 布林值參數現在會正確顯示為勾選框")
    print("2. 支援 true/false、1/0、'true'/'false' 等不同格式的預設值")
    print("3. 類型切換時會保持當前的預設值狀態")
    print("4. 新增參數時布林值預設為不勾選（false）")
