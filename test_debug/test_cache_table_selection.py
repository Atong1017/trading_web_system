...
async def test_cache_table_selection():
    """測試快取表格選擇功能"""
    print("開始測試快取表格選擇功能")
    print("=" * 60)
            ...
            if data.get('status') == 'success':
                ...
                if cache_files:
                    ...
                    for file_info in cache_files:
                        print(f"  - {file_info['display_name']} ({file_info['size_mb']}MB)")
                        print(f"    檔案: {file_info['filename']}")
                        print(f"    類型: {file_info['data_type']}")
                        print(f"    股票: {file_info['stock_id']}")
                        print(f"    日期: {file_info['date_range']}")
                        print()
                else:
                    print("⚠️  沒有找到快取檔案")
...
def test_strategy_with_cache_table(cache_files):
    """測試使用快取表格的策略測試"""
    print("\n2. 測試使用快取表格的策略測試")
...
def should_exit(stock_data, current_index, position):
    ...
    return False, {}"""
        ...
        # 添加快取檔案測試案例
        for i, file_info in enumerate(cache_files[:3]):  # 只測試前3個檔案
            test_cases.append({
                "name": f"使用快取檔案 {i+1}",
                "data_type": "daily_price",
                "stock_id": "2330",
...
async def main():
    """主測試函數"""
    # 1. 測試取得快取檔案列表
    cache_files = await test_cache_table_selection()
    
    # 2. 測試使用快取表格的策略測試
    test_strategy_with_cache_table(cache_files)
