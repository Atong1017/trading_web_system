...
def print_log(message):
    print(f"********** test_data_merge.py - {message}")
...
def test_data_merge():
    ...
    try:
        ...
        test_df.write_excel(temp_excel_path)

        excel_df = pl.read_excel(temp_excel_path)
        ...
        print(11111111, excel_df)

        excel_df = excel_df.with_columns(pl.col("date").cast(pl.Utf8))

        for i in range(len(excel_df)):
            current_row = excel_df.row(i, named=True)
            
            print(11122222222222222, current_row)

        # 合併資料（使用stock_id和date作為合併鍵）
        merged_df = excel_df.join(
            cache_df,
            on=["stock_id", "date"],
            how="inner"
        )
